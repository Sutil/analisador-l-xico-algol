...
{
    Ty_ty p = (Ty_ty)checked_malloc(sizeof(*p));
    p->kind = Ty_ty_::Ty_funcWithFuncReturn;
...
{
    Ty_ty p = (Ty_ty)checked_malloc(sizeof(*p));
    p->kind = Ty_ty_::Ty_funcWithSimpReturn;
    p->parameter = parameter;
    p->returns = returns;
    return p;
}
...
int Ty_is_compatible(Ty_ty ty1, Ty_ty ty2)
{
    if (ty1 == ty2) {
        return 1;
    } else if ((ty1->kind == Ty_ty_::Ty_record && ty2->kind == Ty_ty_::Ty_nil) ||
               (ty1->kind == Ty_ty_::Ty_nil && ty2->kind == Ty_ty_::Ty_record)) {
        return 1;
    } else {
        return 0;
    }
